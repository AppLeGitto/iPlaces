...
//  MyPlaces
//
//  Created by Марк Кобяков on 25.04.2022.
//
...
class MapManager {
    ...
    func checkLocationServices(mapView: MKMapView, segueIdentifier: String, closure: () -> ()) {
        ...
        if CLLocationManager.locationServicesEnabled() {
            ...
            checkLocationAutorization(mapView: mapView, incomeSegueIdentefier: segueIdentifier)
            closure()
        } else {
    ...
    func checkLocationAutorization(mapView: MKMapView, incomeSegueIdentefier: String) {
        switch CLLocationManager.authorizationStatus() {
        case .authorizedWhenInUse:
            mapView.showsUserLocation = true
            if incomeSegueIdentefier == "getAddress" {showUserLocation(mapView: mapView)}
            break
        case .denied:
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                self.showAlert(title: "Your location is not availeble",
